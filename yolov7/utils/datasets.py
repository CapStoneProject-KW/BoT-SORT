...
class LoadImages:  # for inference
    ...
    def __next__(self):
        # stop iteration if it's last video/image
        if self.count == self.nf:
            raise StopIteration
        # the path of the last data
        ...
        if self.video_flag[self.count]:
            # Read video
            self.mode = 'video'
            # ret_val: True if succeeded in reading frame else False
            # img0: Read frame (image)
            ret_val, img0 = self.cap.read()
            # Failed reading frame
            if not ret_val:
                self.count += 1 # move to next video
                self.cap.release()
                if self.count == self.nf:  # last video
                    raise StopIteration
                else:           # if video is left
                    # video path
                    path = self.files[self.count]
                    self.new_video(path)
                    ret_val, img0 = self.cap.read()
            if self.frame % 10 == 0:
                print(f'video {self.count + 1}/{self.nf} ({self.frame}/{self.nframes}) path: "{path}"')
            # next frame
            self.frame += 1
        # If images, 
        else:
